...
var guardarDificultad;
var guardarModeloNave;
var guardarModeloLuna;
...
window.onload = function arrancarJuego(){
    ...
    $.ajax({
        ...
        success: function (jsn) {
            cambiarDificultad(jsn.dificultad);
            cambiarModeloLuna(jsn.modeloLuna);
            cambiarModeloNave(jsn.modeloNave);
	...
	document.getElementById("modeloNave").onclick = function cambiarModeloNave(){
		switch(modeloNave) {
			case 1:
				...
				document.getElementById("modeloNave").innerHTML="PodRacer";
				modeloNave=2;
                                guardarModeloNave="PodRacer";
				restart();
				break;
			case 2:
				document.getElementById("imgNave").src="img/nave.png";
				document.getElementById("imgMotor").src="img/motor.gif";
				document.getElementById("modeloNave").innerHTML="Estándar";
				modeloNave=1;
                                guardarModeloNave="Estándar";
				restart();
				break;
			}
		}
...
function guardarConfig() {
    ...
    $.ajax({
        method: "POST",
        url: url,
        data: {dificultad: guardarDificultad, modeloLuna: guardarModeloLuna, modeloNave: guardarModeloNave},
        success: function (u) {
            alert(u["mess"]);
        },
        error: function (e) {
...
//Seleccionar nave
    ...
    function  cambiarModeloNave(nav) {
    switch (nav) {
        case "PodRacer":
...
function cambiarModeloLuna(lun) {
    switch (lun) {
        case "Gris":
            lunaInicial = lunaActual;
            lunaActual = "Gris";
            break;
        case "Amarilla":
            lunaInicial = lunaActual;
...
function  cambiarDificultad(difAux) {
    var msj = "";
    switch (difAux) {
        case "Fácil":
            fuelInicial = 100;
